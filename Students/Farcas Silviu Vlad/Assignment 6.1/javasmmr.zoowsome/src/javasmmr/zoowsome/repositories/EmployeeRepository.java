...
public class EmployeeRepository extends EntityRepository<Employee> {
	...
	protected Employee getEntityFromXmlElement(Element element){
		...
		case Constants.Employee.Caretaker:
			Employee caretaker = new Caretaker(String.valueOf(element.getElementsByTagName("name").item(0).getTextContent()), Long.valueOf(element.getElementsByTagName("id").item(0).getTextContent()), BigDecimal.valueOf(Long.valueOf(element.getElementsByTagName("salary").item(0).getTextContent())));
